...
void CompressorNode::declare_params()
{
  ...
  // Stage 3+: add coordinator profile params here (default_speed_rpm, eco_cpm,
  // performance_cpm). See architecture.md §6 for the mode → setpoint translation design.
}
...
void CompressorNode::load_params()
{
  ...
  blackboard_->set("service_name", std::string("/") + get_name() + "/compressor_cmd");
  
  // Architecture contracts (hardcoded)
  blackboard_->set("low_booster_action",  std::string("/low_booster/control_booster"));
  blackboard_->set("high_booster_action", std::string("/high_booster/control_booster"));
  ...
  blackboard_->set("interstage_pt_index", interstage_pt_index);
  ...
  blackboard_->set("interstage_sv_index", interstage_sv_index);
