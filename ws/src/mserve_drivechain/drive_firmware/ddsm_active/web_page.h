...
<style type="text/css">
    ...
    .cmd-log-row{
        display: grid;
        grid-template-columns: 70px 58px 76px minmax(0, 1fr);
        gap: 8px;
        padding: 8px 10px;
        border-bottom: 1px solid rgba(194,196,201,0.10);
        font-family: Consolas, monospace;
        font-size: 12px;
        line-height: 1.35;
    }
    .cmd-log-row:last-child{border-bottom: 0;}
    .cmd-log-src{color: #1EA1FF;}
    .cmd-log-status{color: rgba(255,255,255,0.75);}
    .cmd-log-raw{
        color: rgba(255,255,255,0.8);
        word-break: break-all;
...
<body>
                    ...
                    <div class="fb-input-info">
                        ...
                        <div class="cmd-log">
                            <div class="cmd-log-head">
                                ...
                                <button class="btn-of btn-main-s" onclick="refreshCmdLog();">REFRESH</button>
                            </div>
                            <div id="CmdLogList" class="cmd-log-list">
                                <div class="cmd-log-empty">No commands logged.</div>
                            </div>
                        </div>
                        <div class="serial-stats">
                            <div class="cmd-log-head">
                                <span>SERIAL STATS</span>
                                <button class="btn-of btn-main-s" onclick="serialPrintTest();">PRINT TEST</button>
                            </div>
                            <div class="serial-stats-grid">
                                <span>RX bytes</span><span id="SerialRxBytes" class="serial-stats-value">0</span>
                                <span>RX lines</span><span id="SerialRxLines" class="serial-stats-value">0</span>
                                <span>Idle ms</span><span id="SerialIdleMs" class="serial-stats-value">-</span>
                                <span>Last chars</span><span id="SerialLastChars" class="serial-stats-value">-</span>
                                <span>Current</span><span id="SerialCurrentLine" class="serial-stats-value">-</span>
                            </div>
                        </div>
                        <div class="Servo-set">
                            <p>WIFI SETTINGS</p>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            refreshCmdLog();
            refreshSerialStats();
            setInterval(refreshCmdLog, 1500);
            setInterval(refreshSerialStats, 1500);
        });
        ...
        function renderCmdLog(entries) {
            var list = document.getElementById("CmdLogList");
            if (!entries || entries.length === 0) {
                list.innerHTML = '<div class="cmd-log-empty">No commands logged.</div>';
                return;
            }
            ...
            for (var i = entries.length - 1; i >= 0; i--) {
                var entry = entries[i];
                var seconds = ((entry.ms || 0) / 1000).toFixed(1) + "s";
                var cmdType = entry.T === undefined ? "" : "T=" + entry.T;
                html += '<div class="cmd-log-row">' +
                    '<span>' + escapeHtml(seconds) + '</span>' +
                    '<span class="cmd-log-src">' + escapeHtml(entry.src || "") + '</span>' +
                    '<span class="cmd-log-status">' + escapeHtml(entry.status || "") + '</span>' +
                    '<span class="cmd-log-raw">' + escapeHtml((cmdType ? cmdType + " " : "") + (entry.raw || "")) + '</span>' +
                    '</div>';
            }
            list.innerHTML = html;
        }

        function refreshCmdLog() {
            var xhttp = new XMLHttpRequest();
            xhttp.onreadystatechange = function() {
                if (this.readyState == 4 && this.status == 200) {
                    try {
                        renderCmdLog(JSON.parse(this.responseText));
                    } catch (e) {
                        document.getElementById("CmdLogList").innerHTML =
                            '<div class="cmd-log-empty">Log unavailable.</div>';
                    }
                }
            };
            xhttp.open("GET", "cmdlog", true);
            xhttp.send();
        }
        ...
        function refreshSerialStats() {
            var xhttp = new XMLHttpRequest();
            xhttp.onreadystatechange = function() {
                if (this.readyState == 4 && this.status == 200) {
                    try {
                        var stats = JSON.parse(this.responseText);
                        setText("SerialRxBytes", stats.rx_bytes);
                        setText("SerialRxLines", stats.rx_lines);
                        setText("SerialIdleMs", stats.idle_ms);
                        setText("SerialLastChars", stats.last_chars);
                        setText("SerialCurrentLine", stats.current_line);
                    } catch (e) {
                        setText("SerialLastChars", "Stats unavailable.");
                    }
                }
            };
            xhttp.open("GET", "serialstats", true);
            xhttp.send();
        }

        function serialPrintTest() {
            var xhttp = new XMLHttpRequest();
            xhttp.onreadystatechange = function() {
            ...
            xhttp.open("GET", "serialtest", true);
            xhttp.send();
        }
