...
// Liveness heartbeat — separate from heartbeat_time_ms above. This is an
// outgoing "I'm alive" message (FB_HEARTBEAT, T:20020) sent unconditionally
// on a fixed interval so the Pi can detect the board is present and running.
...
void loop() {
  // ddsm motor-stop watchdog.
  heartbeat_ctrl();

  // periodic "board alive" liveness heartbeat over UART.
  uartHeartbeat();
