#!/usr/bin/env python3
"""Test: LOW booster boost to target pressure.

Pass: compressor goal SUCCEEDS, low outlet pressure reaches target.
Fail: goal rejected, timeout, or booster reports failure.
"""
import sys
import rclpy
...
class TestLow(Node):
    ...
    def run(self) -> bool:
        self.get_logger().info(f'--- test_low: LOW START → {TARGET_BAR} bar ---')
        ...
        goal.target         = CC.Goal.LOW_BOOSTER
        ...
        rclpy.spin_until_future_complete(self, result_f, timeout_sec=90.0)
