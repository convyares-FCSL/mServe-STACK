...
if [[ "$USE_DOCKER" == true ]]; then
  ...
  # being snapshotted once at container-create time. This also means the
  # container survives fine even if a peripheral (e.g. the lidar) is
  # unplugged — only that one node fails to configure, not the whole stack.
